...
df_train = pd.read_csv('/home/kaggleLLAM/all_12_with_softlabel_context2.csv')
# print(len(df_train))
...
def preprocess(example):
    ...
    if example.get('softlabel'):
        softlabel = np.fromstring(example['softlabel'][1:-1], float, sep=' ')
        tokenized_example['label'] = np.append(softlabel, tokenized_example['label'])
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        try:
            if len(labels[0]) == 1:
                ...
                Loss_fn = nn.CrossEntropyLoss(reduction="mean")
                loss1 = Loss_fn(input=outputs.view(len(labels),-1), target=labels.view(-1).long()).requires_grad_(True)
                loss2 = Loss_fn(input=outputs.view(len(labels),-1), target=softlabels.view(len(labels), -1)).requires_grad_(True)
                loss3 = torch.mean(map_at3_loss(outputs.view(len(labels),-1), labels.view(-1).long())).requires_grad_(True)
                loss = loss1 + loss2 + loss3
                
                wandb.log({'loss1': loss1, 'loss2': loss2, 'loss3': loss3})
        except:
            print('warning!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            print('inputs', inputs)
...
if __name__ == "__main__":
    ...
    tokenized_dataset = dataset.map(preprocess, remove_columns=['prompt', 'context', 'A', 'B', 'C', 'D', 'E', 'answer', 'softlabel'])
